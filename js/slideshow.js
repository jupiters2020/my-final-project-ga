define([
  __cargo_context__ === "staging" ? "/js/base.js" : "js/base.js",
  "text!/_jsapps/imagegallery/slideshow/defaults.json",
], function (GalleryBase, defaults) {
  return GalleryBase.extend({
    ...
    setElAttributes: function () {
      ...
      this.$el.removeAttr(
        "image-gallery-horizontal-align image-gallery-vertical-align image-gallery-pad image-gallery-gutter data-exploded image-gallery-row data-slideshow-in-transition"
      );
    ...
    initialize: function (options) {
      ...
      if (options.galleryOptions) {
        ...
        this.galleryOptions.data = _.defaults(
          model_data || {},
          JSON.parse(defaults)
        );
      }
      ...
      if (
        typeof scrollMonitor !== "undefined" &&
        $(this.el).closest(".pinned").length == 0
      ) {
    ...
    pause: function () {
      if (
        this.$el.hasClass("slick-initialized") &&
        this.galleryOptions.data.autoplay
      ) {
    ...
    handleUpdates: function (galleryOptions, options) {
      ...
      switch (options.changing) {
        ...
        case "transition-type":
          ...
          if (this.galleryOptions.data["transition-type"] === "scrub") {
            ...
            this.$el.slick(
              "slickSetOption",
              "speed",
              parseFloat(this.galleryOptions.data.speed) * 1000
            );
          }

          if (
            options.value == "slide" &&
            parseFloat(this.galleryOptions.data.speed) != 0
          ) {
          ...
          this.$el.slick(
            "slickSetOption",
            "fade",
            options.value == "scrub" || options.value == "fade",
            true
          );
        ...
        case "speed":
          ...
          if (
            options.value == 0 &&
            this.$el.slick("getSlick").options.fade == false
          ) {
            this.$el.slick("getSlick").$slideTrack.finish();
            this.$el.slick("getSlick").$slides.finish();
            ...
            galleryOptions.data["transition-type"] != "fade" &&
            this.$el.slick("getSlick").options.fade == true
          ) {
            this.$el.slick("getSlick").$slideTrack.finish();
            this.$el.slick("getSlick").$slides.finish();
    ...
    render: function () {
      if (
        Cargo.Core.ImageGallery.draggingInEditor &&
      ...
      _.each(this.images, function (imageObject, index) {
        ...
        var interiorImages = image.querySelectorAll(
          "img[width][height], iframe[width][height], video[width][height]"
        );
        ...
        _.each(interiorImages, function (interiorImage) {
          ...
          $(interiorImage).removeAttr(
            "data-elementresizer-no-resize data-elementresizer-no-centering data-elementresizer-no-vertical-resize"
          );
        });
        ...
        if (
          ...
          !image.hasAttribute("data-elementresizer-child")
        ) {
          $(image).removeAttr(
            "data-elementresizer-no-resize data-elementresizer-no-centering data-elementresizer-no-vertical-resize"
          );
        ...
        if (image.tagName === "A" && interiorImages.length == 1) {
          ...
          ratio =
            interiorImages[0].getAttribute("height") /
            interiorImages[0].getAttribute("width");
        } else {
      ...
      this.setAlignmentAttributes();
      this.$el.prepend(
        '<div class="slideshow-nav" style="display: none" contenteditable="false"><a href="#" data-prev>Prev</a> / <a href="#" data-next>Next</a> (<span data-current data-ignore-changes>1</span> of <span data-total data-ignore-changes>1</span>)</div>'
      );
      ...
      this.listenTo(
        Cargo.Event,
        "elementresizer_update_complete",
        this.debouncedSlickUpdate
      );
    ...
    setConstraint: function () {
      ...
      if (this.galleryOptions.data.constrain_height) {
        ...
        for (var i = 0; i < images.length; i++) {
          ...
          var ratio =
            images[i].getAttribute("width") / images[i].getAttribute("height");
          var scale = images[i].hasAttribute("data-scale")
            ? parseInt(images[i].getAttribute("data-scale")) * 0.01
            : 1;
          var iconMode = images[i].hasAttribute("data-icon-mode");
          if (
            Cargo.Model.DisplayOptions.get("layout_options")
              .mobile_images_full_width &&
            Cargo.Helper.IsMobileWindowSize(baseUnit.cache.window.w) &&
            !iconMode
          ) {
            scale = 1;
          }
          ...
          if (iconMode) {
            maxWidth = Math.min(
              maxWidth,
              scale * baseUnit.cache.size * 16 * ratio
            );
          } else {
    ...
    focusSlide: function () {
      ...
      var selectedItem = CargoEditor.helpers.getAllEffectivelyContainedNodes(
        ...
        _.bind(function (node) {
          nodeInsideSlideShow =
            (this.el.contains(node) && document.contains(node)) ||
            nodeInsideSlideShow;
          return (
            node.nodeName === "IMG" ||
            node.nodeName === "VIDEO" ||
    ...
    explodeView: function () {
      ...
      _.each(this.images, function (imageObject, index) {
        ...
        $(cardInner).attr({
          ...
          style:
            ...
            (parseInt(imageObject.height) / parseInt(imageObject.width)) * 100 +
            "%",
      ...
      var newHeight = Math.max(
        this.$el.height(),
        parseInt(this.el.getAttribute("data-drag-height"))
      );
    ...
    getThumbRectPositionRelatedToPoint: function (point, rect) {
      ...
      if (in_x && in_y) {
        ...
        midpoint_distance = Math.sqrt(
          midpoint_rise * midpoint_rise + midpoint_run * midpoint_run
        );
      } else {
        if (below) {
    ...
    indicateInsertion: function (event, dragged, dragRect) {
      ...
      for (var i in this.parentView.cachedRects.rects) {
        ...
        var positions = this.getThumbRectPositionRelatedToPoint(
          m,
          this.parentView.cachedRects.rects[i]
        );
        ...
        if (
          this.parentView.cachedRects.rects[i].positions.above &&
          this.parentView.cachedRects.rects[i].positions.distance < minDistAbove
        ) {
          minDistAbove = this.parentView.cachedRects.rects[i].positions
            .distance;
          closestThumbAbove = i;
        }

        if (
          this.parentView.cachedRects.rects[i].positions.below &&
          this.parentView.cachedRects.rects[i].positions.distance < minDistBelow
        ) {
          minDistBelow = this.parentView.cachedRects.rects[i].positions
            .distance;
          closestThumbBelow = i;
        }

        if (
          this.parentView.cachedRects.rects[i].positions.to_left &&
          this.parentView.cachedRects.rects[i].positions.distance <
            minDistToLeft
        ) {
          minDistToLeft = this.parentView.cachedRects.rects[i].positions
            .distance;
          closestThumbToLeft = i;
        }

        if (
          this.parentView.cachedRects.rects[i].positions.to_right &&
          this.parentView.cachedRects.rects[i].positions.distance <
            minDistToRight
        ) {
          minDistToRight = this.parentView.cachedRects.rects[i].positions
            .distance;
          closestThumbToRight = i;
        }
      }

      var targetNext = (targetPrev = horizVertical = indicatePrev = indicateNext =
        "default");

      if (this.parentView.cachedRects.rects[closestThumb].midPoint.x > m.x) {
        // now to figure out where it gets indiciated
        var prevItem = $(
          '[data-gallery-item-id="' + (parseInt(closestThumb) - 1) + '"]'
        );

        if (prevItem.length > 0) {
          if (
            (this.parentView.cachedRects.rects.hasOwnProperty(
              prevItem.attr("data-gallery-item-id")
            ) &&
              this.parentView.cachedRects.rects[
                prevItem.attr("data-gallery-item-id")
              ].midPoint.x <
                this.parentView.cachedRects.rects[closestThumb].midPoint.x) ||
        ...
        if (nextItem.length > 0) {
          if (
            (this.parentView.cachedRects.rects.hasOwnProperty(
              nextItem.attr("data-gallery-item-id")
            ) &&
              this.parentView.cachedRects.rects[
                nextItem.attr("data-gallery-item-id")
              ].midPoint.x >
      ...
      this.$el
        .find(".indication-prev, .indication-next")
        .removeClass("indication-prev indication-next");

      if (indicatePrev != "default") {
        this.$el
          .find('[data-gallery-item-id="' + indicatePrev + '"]')
          .addClass("indication-prev")
          .css({
      ...
      if (indicateNext != "default") {
        this.$el
          .find('[data-gallery-item-id="' + indicateNext + '"]')
          .addClass("indication-next")
          .css({
      ...
      var galleryCards = this.$el
        .find(".gallery_card")
        .not(".indication-next, .indication-prev");
    ...
    initSlick: function () {
      ...
      var slickOptions = {
        autoplay:
          this.galleryOptions.data["transition-type"] === "scrub"
            ? false
            : autoplay,
        ...
        speed:
          this.galleryOptions.data["transition-type"] === "scrub"
            ? 0
            : model_data.speed * 1000,
        fade:
          model_data["transition-type"] == "fade" ||
          model_data.speed == 0 ||
          this.galleryOptions.data["transition-type"] === "scrub",
        ...
        prevArrow:
          '<div class="slick-prev image-gallery-navigation">\
        ...
        nextArrow:
          '<div class="slick-next image-gallery-navigation">\
        ...
        dots: false,
        variableWidth:
          this.galleryOptions.data["transition-type"] == "slide" &&
          this.galleryOptions.data.speed != 0,
        lazyLoad:
          this.galleryOptions.data["transition-type"] === "scrub" &&
          this.images.length <= 32
            ? "progressive"
            : "ondemand",
      ...
      var resumeVideos = function (slick) {
        ...
        var videos = slick.$slides[currentSlide].querySelectorAll(
          "video[data-autoplay]"
        );
      ...
      var onMouseMove = function (event) {
        ...
        if (_this.galleryOptions.data["transition-type"] == "scrub") {
          ...
          var targetSlide =
            (startSlideIndex + slideMoveDelta + numberOfSlides) %
            numberOfSlides;
      ...
      var bindScrubNav = function () {
        ...
        $nextArrow.off(
          "mousedown.scrub touchstart.scrub mouseup.nav mouseleave.nav touchend.nav"
        );
        if (_this.galleryOptions.data["transition-type"] !== "scrub") {
          return;
        }
      ...
      var onHoldNextNav = function () {
        holdDelta = Math.max(20, Math.min(250, holdDelta * 0.9));
      ...
      var decayAutoPlay = function (delta, iteration) {
        var newDelta = delta * 1.05 + iteration * 0.5;
        ...
        if (
          model_data.autoplay &&
          newDelta >=
            Math.min(0.25, parseFloat(model_data.autoplaySpeed)) * 1000
        ) {
          autoplayTimeout = setTimeout(function () {
            scrubAutoplay();
      ...
      var endMouseMove = function (event) {
        ...
        if (_this.galleryOptions.data["transition-type"] !== "scrub") {
          ...
          if (timeDelta < 100) {
            autoplayTimeout = setTimeout(function () {
              decayAutoPlay(timeDelta, timeDelta * 0.5);
            }, timeDelta);
          }
      ...
      $slideshow
        ...
        .on("afterChange", function (event, slick, currentSlide) {
          var videoIframe = slick.$slides[lastSlideIndex].querySelector(
            "iframe"
          );
        ...
        .on(isAdminEdit ? "dblclick" : "click", ".slick-list > *", function (
          e
        ) {
          if (!mouseMoved && !cancelClick) {
            if (
              e.target.classList.contains("image-zoom") ||
        ...
        .on("beforeChange", function (event, slick, currentSlide, nextSlide) {
          ...
          var currentTime = 0;
          _.each(
            slick.$slides[currentSlide].querySelectorAll("video"),
            function (videoEl) {
              videoEl.pause();
      ...
      if (this.singleImage) {
        $slideshow
          ...
          .attr("data-exclude-item", "");
      }

      if (isAdminEdit) {
        $slideshow
          .find(".slick-cloned *, .slick-cloned")
          .attr("data-exclude-item", "");
      }
      ...
      if ($slides && $slides.length > 0) {
        ...
        _.each(
          $slideshow
            .slick("getSlick")
            .$slides[currentSlide].querySelectorAll("video[data-autoplay]"),
          function (videoEl) {
            var playPromise = videoEl.play();
    ...
    updateSlick: function () {
      ...
      var anchorImage = this.$el
        .find(
          '.slick-slide:not(.slick-cloned) *[data-gallery-uid="' +
            this.anchorUID +
            '"]'
        )
        .get(0);
      ...
      if (this.galleryOptions.data.constrain_height && anchorImage) {
        ...
        if (this.anchorHeight != anchorHeight) {
          ...
          for (var i = 0; i < anchoredImages.length; i++) {
            ...
            anchoredImages[i].style.width =
              Math.min(newWidth, Math.round(elRect.width)) + "px";
      ...
      for (var i = 0; i < captions.length; i++) {
        ...
        maxCaptionHeight = Math.max(
          maxCaptionHeight,
          parseFloat(captionStyle.getPropertyValue("height")) +
      ...
      list.style.height =
        maxImageHeight + maxCaptionHeight + cardPadHeight + "px";
      ...
      if (this.galleryOptions.data.constrain_height) {
        ...
        if (scale != 100 && targetImage && this.anchorUID) {
          ...
          if (targetUID == this.anchorUID) {
            ...
            this.$el
              .find('[data-gallery-uid="' + targetUID + '"]')
        ...
        var targetIndex = parseInt(
          $(targetImage)
            .closest("[data-gallery-item-id]")
            .attr("data-gallery-item-id")
        );
        var targetMid = targetImage.getAttribute("data-mid");

        var currentScale = targetImage.hasAttribute("data-scale")
          ? parseInt(targetImage.getAttribute("data-scale"))
          : 100;
