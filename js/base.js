define([], function () {
  return Backbone.View.extend({
    ...
    createItem: function (itemDescriptor, clean) {
      ...
      switch (itemDescriptor.type) {
        ...
        case "image_link":
          ...
          if (!clean) {
            interiorItem.setAttribute(
              "data-gallery-item-index",
              itemDescriptor.index
            );

            if (!_.isEmpty(itemDescriptor.meta)) {
              interiorItem.setAttribute(
                "data-meta",
                JSON.stringify(itemDescriptor.meta)
              );
            }
          }
    ...
    itemDescriptors: [
      {
        ...
        matches: function (el) {
          return (
            ...
            !el.hasAttribute("data-exclude-item")
          );
        },
      },
...

      {
        ...
        matches: function (el) {
          return (
            ...
            el.hasAttribute("height") &&
            !el.hasAttribute("data-exclude-item") &&
            (el.hasAttribute("src") || el.hasAttribute("data-mid"))
          );
        },
      },

      {
        ...
        findChildren: function (el) {
          var firstImage = el.querySelectorAll(
            "img[width][height], video[width][height], iframe[width][height]"
          )[0];
    ...
    compileItemDescriptors: function () {
      ...
      _.each(
        this.itemDescriptors,
        function (descriptor) {
          this.allItemTagNames = _.union(this.allItemTagNames, [
            descriptor.tagName,
          ]);
        },
        this
    ...
    getGalleryItems: function () {
      ...
      var candidateElements = this.el.querySelectorAll(
          this.allItemTagNames.join(",")
        ),
        results = [];
    ...
    serialize: function () {
      ...
      _.each(items, function (item, index) {
        ...
        if (item.type === "image_link") {
          ...
          autoplay =
            item.children[0].hasAttribute("autoplay") ||
            item.children[0].hasAttribute("data-autoplay");
          controls = item.children[0].hasAttribute("controls");
          playsinline =
            item.children[0].hasAttribute("playsinline") ||
            item.children[0].hasAttribute("webkit-playsinline");

          // product links
          productID = item.el.hasAttribute("data-product")
            ? item.el.getAttribute("data-product")
            : undefined;
          skuID = item.el.hasAttribute("data-sku")
            ? item.el.getAttribute("data-sku")
            : undefined;

          markerID =
            item.children[0] && item.children[0].hasAttribute("data-marker-id")
              ? item.children[0].getAttribute("data-marker-id")
              : undefined;
          ...
          if (item.el.hasAttribute("data-gallery-item-index")) {
            itemIndex = parseInt(
              item.el.getAttribute("data-gallery-item-index")
            );
            ...
            itemIndex = parseInt(
              item.children[0].getAttribute("data-gallery-item-index")
            );
          } else {
            itemIndex = index;
          }
          ...
          if (item.children[0].hasAttribute("data-meta")) {
            meta = Cargo.Core.ImageGallery.parseGalleryOptions(
              item.children[0].getAttribute("data-meta")
            );
          }
          ...
          autoplay =
            item.el.hasAttribute("autoplay") ||
            item.el.hasAttribute("data-autoplay");
          controls = item.el.hasAttribute("controls");
          playsinline =
            item.el.hasAttribute("playsinline") ||
            item.el.hasAttribute("webkit-playsinline");

          markerID = item.el.hasAttribute("data-marker-id")
            ? item.el.getAttribute("data-marker-id")
            : undefined;
          ...
          if (item.el.hasAttribute("data-meta")) {
            meta = Cargo.Core.ImageGallery.parseGalleryOptions(
              item.el.getAttribute("data-meta")
            );
          }
